...
local doNotSkip = { [57] = true, [1022] = true, [1023] = true, [1024] = true, [1025] = true, [1027] = true, [1029] = true }
...
local alpha_voice =
{
		...
		for k, agent in pairs( observers ) do			
			if agent and #allEnemies > 0 then
				for i, enemy in pairs(allEnemies) do
					...
					and not enemy:isKO() and ( not enemy:isAlerted() or ( getAlerted == true) and not enemy:isAiming() ) 
	...
	checkGuardType = function( chattyguard )
		...
		elseif chattyguard:isAlerted() then --no need for params check, getGuards does that
			guardtype = "type_guard_alerted"
		else
			guardtype = "type_guard"
		end
		if chattyguard:getBrain() and tostring( (chattyguard:getBrain():getSituation()["ClassType"]) == simdefs.SITUATION_FLEE) then
	...
	-- chatty guard block --
		...
		if evType == simdefs.TRG_START_TURN and evData:isPC() then
			...
			local p_banter_hunting = 0.1
			if sim.chattyGuards == nil then 
				sim.chattyGuards = sim:getTurnCount()
			end
			-- log:write("talkative guards 1")
			if sim:getTurnCount() > sim.chattyGuards and (sim:getTrackerStage( sim:getTracker() ) < 5) then --no guard lines post 'full alert' alarm levels, no matter the settings. It's srs bsness time!
